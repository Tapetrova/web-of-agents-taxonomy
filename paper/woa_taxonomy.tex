...
\textbf{Relation to prior work.}  An extended descriptive review of the Semantic Web, MAS, and LLM-agent lineage, together with a per-framework comparison, is provided in~\cite{Petrova_WoA_Survey_2025}; the present article focuses more narrowly on the taxonomy itself as an analytical instrument and adds the operational codebook and validation protocol that the earlier review did not include.  Recent protocol taxonomies classify along technical dimensions (e.g., feature tables comparing MCP, A2A, ACP, ANP).  None, to our knowledge, report inter-rater reliability for their axis assignments, or link taxonomic position to measurable adoption outcomes.  The present article differs in these two respects.
